...
echo "LOAD, sampled in slot. NOT the energy of one launch. The idle baseline and" | tee -a "$LOG"
echo "each slot's pre-idle reading are DIAGNOSTICS: on a DVFS board an idle slot" | tee -a "$LOG"
echo "is a different clock state, so the analysis FITS the busy-state floor from" | tee -a "$LOG"
