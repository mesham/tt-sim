// Op test for `reduce_tile` (host side). Feeds one bfloat16 data tile plus a
...
// PoolType::MAX lowers to GMPOOL, SUM/AVG to GAPOOL, and REDUCE_ROW / SCALAR
// additionally route the pooled result through SrcB (MOVD2B + TRNSPSRCB +
// MOVB2A) to turn a row result into a column.
//
...
int main(int argc, char** argv) {
    ...
    // Data tile: multiples of 0.5 in [0, 20], all exactly representable in
    // bfloat16, and (i*37 % 41) is a permutation cycle so every row and every
    // column has a distinct maximum.
    std::vector<bfloat16> src_vec(TILE_ELEMS);
    for (uint32_t i = 0; i < TILE_ELEMS; i++) {
        src_vec[i] = bfloat16(0.5f * static_cast<float>((i * 37) % 41));
    }
