...
# Repo root must be on PYTHONPATH so `driver.blackhole.server` resolves.
...
echo "[run.sh] starting tt-sim Blackhole server on $NNG_SOCKET_ADDR" >&2
exec python3 -u -m driver.blackhole.server "${extra[@]}" "$@"
