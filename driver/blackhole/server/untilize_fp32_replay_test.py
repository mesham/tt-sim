"""Socket-free replay of the "untilize" wire trace in its **fp32-DEST** arm (Blackhole).

The sibling of ``untilize_replay_test.py``, replaying the *same* op-test program
(``optests/untilize``) launched with ``fp32_dest_acc_en = true`` while every
circular buffer stays ``Float16_b`` (``UNTILIZE_FP32=1``, see the run-mode note
at the top of ``optests/untilize/src/untilize.cpp``). That is a 32-bit DEST
feeding a 16-bit pack source format -- the ordinary GEMM configuration the
tt-xftn compiler team runs -- and it needs a conversation of its own: the
compute kernel is the same source, but tt-metal JITs it differently (the pack
config carries ``PCK_DEST_RD_CTRL_Read_32b_data``, the math thread runs the
32-bit DEST path), so the default arm's trace cannot stand in for it.
...
and 960 of 1024 datums wrong on the three ops here, with op 0 -- the *tiled
control*, which shares nothing with untilize but the pack -- landing only the
top-left 8 rows x 16 columns. The unit-level pin is
...
invisible; it shows from row 8 up, where ``Adj32(8) = 16`` and the 16-bit read
returns the low half of row 0 instead. **A 32x32 tile occupies DEST rows 0-63**
(face ``f`` row ``r`` at DEST row ``16f + r``), so this guard covers rows 0-63
and it is the 896 datums per op that come from rows 8-63 that make it bite. A
guard confined to the first eight rows would pass against the broken packer.

op 2 is the one that is not a copy of Wormhole's: ``pack_untilize_dest`` lowers
here to a MOP running one packer interface pair in ``DST_ACCESS_STRIDED_MODE``,
so a PACR's second sixteen datums come from DEST row ``base + 16`` rather than
``base + 1``. That is why the broken packer got 960 rather than 896 wrong on it:
the strided read reaches above row 7 even for the datums a Wormhole PACR would
have taken from the aliasing rows.
...
produce byte-identical output, and importing it makes that a property of the
code rather than of two copies staying in step. tt-sim reproduces this arm
bit-for-bit against ttsim-Blackhole as well (``UNTILIZE_FP32=1
./optests/diff.sh untilize``), which is also how the trace was captured, with
``TT_SIM_RECORD`` pointed at ``traces/untilize_fp32.trace``.

Run:  python3 -m driver.blackhole.server.untilize_fp32_replay_test
"""
...
# (NUM_OPS 32x32 bfloat16 tiles, contiguous, third allocation in the channel at
# (0, 11)) — only the compute config differs between the arms.
DST_DRAM_COORD = (0, 11)
DST_ADDR = 0x594E80
...
def main():
    ...
    print(
        f"blackhole untilize_fp32_replay test OK ({n_msgs} messages; all "
