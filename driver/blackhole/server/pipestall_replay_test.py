"""Socket-free replay of the captured tt-metal "pipestall" wire trace on Blackhole.

``pipestall`` (see ``examples/pipestall``) is the tree's only workload in which a
Tensix backend unit waits on **another core**. Tile A ``(1, 2)`` runs
reader + compute + sender; tile B ``(2, 2)`` runs the writer and hands a credit
...
# Measured on Blackhole: see the printed line; the Wormhole twin sees 1,056. The floor is well
...
def main():
    ...
    print(
        f"blackhole pipestall_replay test OK ({n_msgs} messages; all {DATA_SIZE} "
