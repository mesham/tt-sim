"""Socket-free replay of the captured tt-metal "matmulidx" wire trace on Blackhole.
...
352 for the accumulating one. All are exactly representable in bfloat16, so this
compares bit-exactly rather than by PCC. ``six`` remains the guard for the
matmul arithmetic itself; this one is about which tiles it reads.
...
issuing its UNPACRs (the two config contexts are what let the issuing thread run
a matmul ahead), so from the third matmul on — once the unpacker starts
stalling on the matrix unit — each operand unpacked through the *next* call's
context and read the wrong tile. It went unnoticed because with equal tile
indices both contexts hold the same address.

Run:  python3 -m driver.blackhole.server.matmulidx_replay_test
"""
...
def main():
    ...
    print(
        f"blackhole matmulidx_replay test OK ({n_msgs} messages; all "
