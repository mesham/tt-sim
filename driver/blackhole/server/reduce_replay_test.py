"""Socket-free replay of the captured tt-metal "reduce" wire trace on Blackhole.

``reduce`` is the pooling half of the op-coverage differential test (see
``optests/reduce`` and ``optests/diff.sh``): one bfloat16 data tile plus the
standard reduce-scaler tile are run through five (PoolType, ReduceDim)
combinations at ``MathFidelity::HiFi4``, one output tile each --
MAX/{COL,ROW,SCALAR} and SUM/{COL,SCALAR}.

Between them they exercise most of the reduction pipeline: GMPOOL and the
multi-phase high-fidelity GAPOOL MOP; the unpacker's haloize transpose (how a
row reduce becomes a column-wise pool); the MOVD2B / TRNSPSRCB / MOVB2A dance
that turns a pooled row back into a column through SrcB's rows 16-31 scratch;
and the packer's reduce edge masks, which are the only thing zeroing the Dst
rows the reduction did not write.

As with ``sfpumath`` there is no closed-form golden -- the answer is what the
hardware's fidelity phases and Dst rounding produce -- so the golden is ttsim's
own dump, captured from the vendor reference sim and stored beside the trace in
``traces/reduce.expected``. All 5120 bfloat16 elements must match bit-exactly.

Run:  python3 -m driver.blackhole.server.reduce_replay_test
"""
...
def main():
    ...
    print(
        f"blackhole reduce_replay test OK ({n_msgs} messages; all {DATA_SIZE} "
