...
// Tile A reader (BRISC, NOC 0). Identical in shape to example four's
// reader: pulls chunked int8 inputs from DRAM into local CB0/CB1, then
// hands them off to the TRISC compute kernel via cb_push_back.
...
void kernel_main() {
    ...
    for (uint32_t i = 0; i < num_chunks; i++) {
        ...
        noc_async_read(src0_dram_noc_addr + i * bytes_per_chunk,
                       l1_write_addr_in0, bytes_per_chunk);
        noc_async_read(src1_dram_noc_addr + i * bytes_per_chunk,
                       l1_write_addr_in1, bytes_per_chunk);
