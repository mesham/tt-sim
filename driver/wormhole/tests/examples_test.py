"""Build every wormhole example against tt-metal and run it on the simulator.

Each example under ``driver/wormhole/tests/<name>/src`` is a real tt-metal host
program that validates its own results and exits non-zero on mismatch. This harness
is the "run it like hardware, but the simulator route" test: it builds each example
with CMake (``find_package(TT-Metalium)``) and runs the resulting binary against
tt-sim via UMD's simulation backend (``TT_METAL_SIMULATOR``), asserting the process
exits 0 and prints its success line.

Run standalone (no pytest needed)::

    python3 -m driver.wormhole.tests.examples_test
...
WORMHOLE = Path(__file__).resolve().parent
...
def _reap_servers():
    ...
    subprocess.run(
        ["pkill", "-9", "-f", "[d]river.wormhole.server"],
        check=False,
