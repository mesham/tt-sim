...
int main(int argc, char** argv) {
    ...
    KernelHandle compute_kernel_id = CreateKernel(
        ...
        core,
        ComputeConfig{
            .fp32_dest_acc_en = true,  // Use 32-bit wide entries in dst
            .dst_full_sync_en = false,
            .unpack_to_dest_mode = unpack_to_dest_mode, // Unpacker targets dst (rather than srcA)
    ...
    for (int i=0;i<DATA_SIZE;i++) {
        printf("%f = %f + %f\n", result_data[i], src0_data[i], src1_data[i]);
