...
float pearson_correlation(const std::vector<bfloat16>& a, const std::vector<bfloat16>& b) {
    ...
    for (size_t i = 0; i < n; i++) {
        mean_a += static_cast<float>(a[i]);
        mean_b += static_cast<float>(b[i]);
    }
    mean_a /= n;
    mean_b /= n;
    ...
    double var_b = 0;
    for (size_t i = 0; i < n; i++) {
        double da = static_cast<float>(a[i]) - mean_a;
        double db = static_cast<float>(b[i]) - mean_b;
        cov += da * db;
        var_a += da * da;
        var_b += db * db;
    }
