...
- **It only engages once it can pay for itself.** Importing numba costs ~0.5 s
  and compiling this kernel costs ~3.7 s cold / ~0.5 s from numba's on-disk
  cache. That is pure loss on a workload with a handful of MVMULs, so the
  import is deferred until ``TT_SIM_NUMBA_THRESHOLD`` of them have run (default
  512, about a quarter of a second of numpy at the measured rate).
...
revival" that does not need a free-threaded interpreter.
"""
