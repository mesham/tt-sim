        ...
        if self.unpacker_id == 0:
            if start_row < 4:
                raise NotImplementedError(
                    ...
                    "was never configured: check that a "
                    "`binary_op_init_common` / `compute_kernel_hw_startup` "
                    "runs before any `*_tiles_init`. Every correctly "
                    "configured kernel we have seen arrives here at 64.\n"
                    "\n"
                    "MEASURED: across matmul, matmul_block, reduce and SFPU "
                    "kernels, in both Float16_b and Float32, unpacker 0 "
                    "arrives at outAddr 64 and unpacker 1 at 0. In all of "
                    "them UNP0_ADDR_BASE_REG_1_Base and the three ADC "
                    "counters read zero, so the whole of outAddr is that "
                    "context register -- 64 for unpacker 0, 0 for unpacker 1. "
                    "The constant 4 is those 64 datums.\n"
                    "\n"
                    "This stop has been a correct verdict every time it has "
                    "fired. The one kernel that ever reached it was missing "
                    "`binary_op_init_common`, and on silicon that same kernel "
                    "ran and returned a constant 1.0f for every element -- so "
                    "hardware accepting the configuration is not evidence "
                    "that it is valid. `optests/srcarow/` holds that kernel "
                    "and its one-line control.\n"
                    "\n"
                    "If your init sequence is correct and you still land here, "
                    "that is new and we would like the kernel: it would be the "
                    "first evidence that the 4 rows belong to the context path "
                    "specifically rather than to unpacker 0's output address "
