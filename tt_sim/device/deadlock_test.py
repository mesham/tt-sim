...
# BRISC boots at L1 offset 0 on Wormhole.
BRISC_START = 0x0
...
def _device_with_watchdog(threshold=THRESHOLD):
    """A one-Tensix Wormhole whose watchdog has a test-sized window."""
    device = Wormhole()
    device.reset()
...
def test_fires_on_a_wedged_device(capsys):
    device, coord, _detector = _device_with_watchdog()
...
def test_detection_latency_is_bounded(capsys):
...
def test_quiet_while_every_core_is_in_reset(capsys):
    device, _coord, _detector = _device_with_watchdog()
...
def test_quiet_on_a_loop_whose_period_is_the_sample_interval(capsys):
