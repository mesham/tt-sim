...
// pipestall Tile A reader (BRISC, NOC 0). Pulls chunked int8 inputs from DRAM
// into CB0/CB1. The input CBs are several pages deep on purpose, so this core
// runs ahead of the compute and the unpack thread always has a tile waiting —
// which is what makes the producer's stall land in the *unpacker* rather than
// in a cb_wait_front spin on the RISC-V.
